...
pub async fn transfer_shard(
    ...
    // ToDo: Initialize temporary shard on a remove machine
    {
        ...
        let transferring_shard = shard_holder_guard.remove_shard(shard_id).await;
        if let Some(Shard::Local(local_shard)) = transferring_shard {
            let proxy_shard = ForwardProxyShard::new(local_shard, remote_shard);
            shard_holder_guard
                .add_shard(shard_id, Shard::ForwardProxy(proxy_shard))
                .await;
        } else {
            return Err(CollectionError::service_error(format!(
    ...
    {
        let mut shard_holder_guard = shard_holder.write().await;
        let proxy_shard_opt = shard_holder_guard.remove_shard(shard_id).await;
        if let Some(Shard::ForwardProxy(proxy_shard)) = proxy_shard_opt {
            let (original_shard, remote_shard) = proxy_shard.deconstruct();
            match transfer_result {
                Ok(_) => {
                    shard_holder_guard
                        .add_shard(shard_id, Shard::Remote(remote_shard))
                        .await;
                }
                Err(error) => {
                    shard_holder_guard
                        .add_shard(shard_id, Shard::Local(original_shard))
                        .await;
                    return Err(error);
                }
            }
